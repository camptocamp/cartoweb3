...
Display.prototype.setTool = function(tool) {
  ...
  case "pan":
    ...
    break;
  case "rollover":
    this.mouseAction = new RollOverTool(this);
    break;
  case "draw.point":
 ...
 * @param aDisplay display object
 */
function RollOverTool(aDisplay) {
...
RollOverTool.prototype.onImagemapOver = function(aDisplay, id) {
  aDisplay._map.onImagemapOver(id);
}
RollOverTool.prototype.onImagemapOut = function(aDisplay) {
  aDisplay._map.onImagemapOut();
}
...
DrawLineTool.prototype.onMouseDown = function(aDisplay, ex, ey) {
  ...
  if (!aDisplay.feature) { // new polyline
...
DrawPolygonTool.prototype.onMouseMove = function(aDisplay, ex, ey) {
  if (aDisplay.isDrawing != "line" && aDisplay.feature) {
    // moving line
    var vl = aDisplay.dShape.X.length;
...
DrawPolygonTool.prototype.onDblClick = function(aDisplay, ex, ey) {
  ...
  var vertex = new Vertex(aDisplay.feature.vertices[0].x, aDisplay.feature.vertices[0].y);
  aDisplay.feature.vertices.push(vertex);
...
Display.prototype.fillPolygon = function(aPolygon, status) {
  ...
  if (!pg.pf) {
    ...
    pg.pf.className = polygonfillCN + status;
  } else {
    pfc = false;
    pg.pf.innerHTML = '';
  }
...
/**
 * Adds an imagemap to the display
 */
Display.prototype.useImagemap = function(aImagemap) {
  ...
  image.useMap = "#" + aImagemap;
  image._display = this;
  xAppendChild(this.rootDisplayLayer, image);
  
  var imagemaps = xGetElementsByTagName("MAP");
  for (i = 0; i < imagemaps.length; i++) {
    if (imagemaps[i].name = aImagemap) {
      var imagemap = imagemaps[i];
      continue;
    }
  }
//  var imagemap = xGetElementById(aImagemap);
  for (i = 0; i < imagemap.areas.length; i++) {
    var oneMap = imagemap.areas[i];
    oneMap._display = this;
  }
  
  if (typeof(tt) == 'undefined') tt = xGetElementById('tt');
  this.imagemapCanvas = new jsGraphics("map_imageCanvas");
  this.imagemapCanvas.setStroke(2);
  this.imagemapCanvasHasDrawing = false;
  this.imagemapIsOver = false;
};
...
function display_onmouseup(evt) {
  ...
  if (_display._map.onClic) {
    _display._map.onClic(_display.feature);
  }
...
function imagemapPolyXcoords(coords) {
  var Xcoords = '';
...
function imagemapPolyYcoords(coords) {
  var Ycoords = '';
...
Display.prototype.imagemapSetFocus = function(area) {
  if (this.canvasHasDrawing == true) return true;
  var coords = area.coords.split(',');
  if ((area.shape.toUpperCase() == 'POLY') || (area.shape.toUpperCase() == 'POLYGON')) {
    var Xcoords = imagemapPolyXcoords(coords);
    var Ycoords = imagemapPolyYcoords(coords);
    ...
    this.imagemapCanvas.fillPolygon(pgx,pgy, polygonfillCN + _SEL);
    this.imagemapCanvas.paint();
    this.imagemapCanvas.drawPolygon(pgx,pgy, linepointCN + _SEL);
    this.imagemapCanvas.paint();
  } else if ((area.shape.toUpperCase() == 'CIRCLE')) {
    this.imagemapCanvas.fillEllipse(parseInt(coords[0]) - parseInt(coords[2]),
      parseInt(coords[1]) - parseInt(coords[2]),
      parseInt(coords[2]) * 2, parseInt(coords[2]) * 2, polygonfillCN + _SEL);
    this.imagemapCanvas.paint();
    this.imagemapCanvas.drawEllipse(parseInt(coords[0]) - parseInt(coords[2]),
      parseInt(coords[1]) - parseInt(coords[2]),
      parseInt(coords[2]) * 2, parseInt(coords[2]) * 2, linepointCN + _SEL);
    this.imagemapCanvas.paint();
  }
...
Display.prototype.imagemapClearFocus = function(area) {
  if (this.canvasHasDrawing) this.imagemapCanvas.clear();
  this.canvasHasDrawing = false;
  return true;
}

Display.prototype.onImagemapOver = function(e, area, id) {
  e = new xEvent(e);
  
  area.title = "";
    
//  cancel the default tooltip to show customized one 
//    e.target.setAttribute('title', "");
  if (!this.imageCanvasIsOver) {
//        tt.innerHTML = id;
//        tt.style.display = "block";
    this.imagemapIsOver = area;
    this.imagemapSetFocus(area);
  }
  xMoveTo(tt, e.pageX, e.pageY + 18);
  this.imagemapCanvasHasDrawing = true;
    
  if (this.mouseAction && this.mouseAction.onImagemapOver) {
    this.mouseAction.onImagemapOver(this, id);
  }
}

Display.prototype.onImagemapOut = function(area) {
//    e = new xEvent(e);
//    tt.innerHTML = "";
//    tt.style.display = "none";
  this.imagemapClearFocus(area);
  this.imagemapisOver = false;
  this.imagemapCanvasHasDrawing = false;
    
  if (this.mouseAction && this.mouseAction.onImagemapOut) {
    this.mouseAction.onImagemapOut(this);
  }
}
