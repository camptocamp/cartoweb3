...
function Imagemap() {
  if (typeof(imagemapToolTip) == 'undefined')
    imagemapToolTip = xGetElementById('imagemapToolTip');
}

Imagemap.prototype.addLayer = function(name, attributes) {
  var aLayer = new ImagemapLayer(name, attributes);
  return aLayer;
}

function ImagemapLayer(name, attributes) {
  this.name = name;
  this.attributes = attributes;
  this.features = new Array();
}

ImagemapLayer.prototype.add = function() {
  var feature = new ImagemapFeature(arguments);
  this.features.push(feature);
}

function ImagemapFeature(arguments) {
  this.id = arguments[0];
  this.values = arguments[1].split(',');
}