...
{$debug_request}
  ...
  <div id="leftbar">    
    ...
    <p>
      ...
      {if $locale != $currentLang}<a href="javascript:document.carto_form.action='{$smarty.server.SCRIPT_NAME}?lang={$locale}';FormItemSelected();">{$locale}</a>{else}<strong>{$locale}</strong>{/if}
